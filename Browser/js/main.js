...
const SmartNotesModule = ({ isOpen, onClose }) => {
    ...
    const handleAIPolish = async () => {
        ...
        const prompt = `You are an expert editor. Improve the following study notes. Fix grammar, make it more concise, and format it nicely with markdown. Keep the original meaning:\n\n${note}`;

        let improvedNote = note;
        if (window.electronAPI && window.electronAPI.chatWithLocalAI) {
            improvedNote = await window.electronAPI.chatWithLocalAI(prompt);
        }
